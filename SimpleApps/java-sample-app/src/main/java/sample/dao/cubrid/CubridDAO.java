...
import sample.models.Org;
import sample.models.Organization;

@Repository
public class CubridDAO extends AbstractDAO{
	
	@Autowired 
    @Qualifier("jdbcCubrid") 
	private JdbcTemplate jdbcTemplate;
	
	
	public List<Org> selectAll() {
		return this.jdbcTemplate.query("select * from ORG_TBL", new RowMapper<Org>() {
    ...
    public Map selectOrganization(Organization param){
     ...
     public int updateOrganization(Organization param){
         ...
         args[3] = param.getId();
          ...
          int count = this.jdbcTemplate.update(DAOSql.SQL_UPDATE_ORG, param);
